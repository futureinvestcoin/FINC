...
public:
    ...
    CService addrLocal;
    int nVersion;
    // strSubVer is whatever byte array we read from the finc. However, this field is intended
    // to be printed out, displayed to humans in various forms and so on. So we sanitize it and
    // store the sanitized version in cleanSubVer. The original should be used when dealing with
    // the network or finc types and the cleaned string used when displayed or logged.
