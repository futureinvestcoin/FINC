...
#include "ui_zfinccontroldialog.h"
...
std::list<std::string> ZfincControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZfincControlDialog::listMints;

ZfincControlDialog::ZfincControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZfincControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZfincControlDialog::~ZfincControlDialog()
{
    delete ui;
}

void ZfincControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZfincControlDialog::updateList()
{
...
void ZfincControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZfincControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZfinc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZfincControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZfincControlDialog::GetSelectedMints()
{
...
void ZfincControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
