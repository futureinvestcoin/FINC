                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zFINC ought to be enough for anybody." - Bill Gates, 2017
    ui->zFINCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFINCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzFINC->setEnabled(false);
        ui->pushButtonMintzFINC->setToolTip(tr("zFINC is currently disabled due to maintenance."));

        ui->pushButtonSpendzFINC->setEnabled(false);
        ui->pushButtonSpendzFINC->setToolTip(tr("zFINC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFINCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFINC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFINC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFINC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFINC in ") +
...
void PrivacyDialog::on_pushButtonSpendzFINC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFINC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zFINC
        sendzFINC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFINC
    sendzFINC();
}

void PrivacyDialog::on_pushButtonZfincControl_clicked()
{
    ZfincControlDialog* zfincControl = new ZfincControlDialog(this);
    zfincControl->setModel(walletModel);
    zfincControl->exec();
}

void PrivacyDialog::setZfincControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzfincSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFINC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid finc Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFINCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFINC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FINC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFINC</b>";
    ...
    // use mints from zfinc selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZfincControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZfincControlDialog::GetSelectedMints();
    }
    ...
    // Clear zfinc selector in case it was used
    ZfincControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zfinc Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " finc, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zfinc Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFINC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFINC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFINC "));
    ui->labelzFINCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzFINCSyncStatus->setVisible(fShow);
}
