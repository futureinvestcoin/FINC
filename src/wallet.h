...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 FINC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zFINC send
enum ZerocoinSpendStatus {
    ZFINC_SPEND_OKAY = 0,                            // No error
    ZFINC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFINC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFINC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFINC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFINC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFINC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFINC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFINC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFINC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZFINC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFINC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFINC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFINC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFINC_SPENT_USED_ZFINC = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZfincAutoBackups(bool fEnabled)
    {
