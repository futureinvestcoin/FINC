...
*Setup instructions for a gitian build of finc using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the finc
...
Clone the git repositories for finc and gitian and then checkout the finc version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/finccash/finc-coin.git
cd finc
...
Follow the instructions in [doc/release-process.md](release-process.md) in the finc repository
...
To build finc (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the finc repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/finc/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/finccash/finc-coin
...
For example:
```bash
URL=https://github.com/finccash/finc-coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit finc=${COMMIT} --url finc=${URL} ../finc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit finc=${COMMIT} --url finc=${URL} ../finc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit finc=${COMMIT} --url finc=${URL} ../finc/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/finc-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/finc-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/finc-build.assert
```
